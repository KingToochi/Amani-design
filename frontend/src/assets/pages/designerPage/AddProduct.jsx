...
const AddProduct = ({setHideModal, productList, fetchProduct, imageLimits}) => {
    ...
    const [formField, setFormField] = useState([
        ...
        {element: "input", type:"number", id:"productPrice", name:"productPrice", placeholder:"Price", label:"Price"},
    ])
    const url = BASE_URL
    ...
    const addToFormField = () => {
          ...
          setFormField((prev) => [...prev,
              ...
              {element: "input", type:"number", id:variantPriceId, name:`ProductPrice${newCount}`, placeholder:"Price", label:`Variant${newCount} price`},
          ])
    }


    const handleChange =(e) => {
      const imagePerProductLimit = imageLimit.imagePerProduct
      const selectedFiles = e.target.files
      setFile(selectedFiles)
        if (selectedFiles.length > imagePerProductLimit) {
        alert(`Maximum ${imagePerProductLimit} images allowed`)
        e.target.value = ""
        setFile([])
        setFileName("")
        return
         ...
         setFileName(Array.from(selectedFiles).map(f => f.name).join(', '));
    }
...
const handleSubmit = async(event) => {
    ...
    <div className="max-w-3xl mx-auto p-8 bg-white rounded-[32px] shadow-[0_20px_80px_rgba(15,23,42,0.08)] border border-gray-200">
        ...
        {formField.map((field, index) => {
            ...
            return (
                <div key={field.id} className="space-y-3">
                    ...
                    <p className="text-sm text-slate-500"> {file && file.length > 0 ? `Selected: ${file.length} image(s)` : "Choose images to upload"}</p>
                </div>
              );
            }
            ...
            <div key={field.id} className="space-y-2">
              ...
              <select id={field.id} name={field.name} className="w-full rounded-3xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-700 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100">
                ...
                {field.option.map((option, i) => (
                  <option key={field.id} value={option} className="text-slate-800">
      ...
      <div className="space-y-4">
        ...
        <button type="submit" disabled={isSubmitting} className="w-full rounded-3xl bg-emerald-600 px-6 py-3 text-sm font-semibold text-white transition hover:bg-emerald-700 sm:w-auto">{isSubmitting ? 'Submitting...' : 'Submit'}</button>
      </div>
       :
      <div className="grid gap-3 sm:flex sm:justify-end sm:items-center">
        <button onClick={addToFormField} disabled={isSubmitting}  type="button" className="w-full rounded-3xl border border-slate-300 bg-white px-6 py-3 text-sm font-semibold text-slate-900 transition hover:bg-slate-100 sm:w-auto">Add Variant Option</button>
        <button type="submit" disabled={isSubmitting} className="w-full rounded-3xl bg-slate-900 px-6 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 sm:w-auto">{isSubmitting ? 'Submitting...' : 'Submit'}</button>
      </div>
      }
    </form>
  </div>
